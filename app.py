...
def edit_user(user_id):
    ...
    if request.method == "POST":
        ...
        # Agar new_password diya hai to hash karke set karo
        if new_password:
...
def portal_dashboard():
    ...
    if role == "vendor" and portal_user_id:
        vendor_oid = ObjectId(portal_user_id)
        # koi bhi order jisme items.vendor_id == is vendor
        vendor_orders = list(
            orders.find({"items.vendor_id": vendor_oid}).sort("created_at", -1)
        )

    if role == "delivery" and portal_user_id:
...
def checkout():
    ...
    # NEW: coordinates from frontend (optional)
    lat = data.get("lat")
    lng = data.get("lng")

    # server-side subtotal (trust only backend calc)
    subtotal = 0
    for item in cart:
    ...
    if lat is not None and lng is not None:
        try:
            order["coords"] = {"lat": float(lat), "lng": float(lng)}
